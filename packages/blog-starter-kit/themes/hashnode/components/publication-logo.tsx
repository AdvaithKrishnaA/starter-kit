...
const PublicationLogo = ({ size = 'lg', isPostPage = false }) => {
  const { theme } = useTheme();
  const logoFill = theme === 'dark' ? '#FFFFFF' : '#000000';

  const logoSizes = {
    xs: 'w-44 h-12', // Updated height to 12 for xs
    sm: 'w-44 h-12', // Updated height to 12 for sm
    lg: 'w-64 h-16', // Updated height to 16 for lg
    xl: 'w-64 h-16', // Updated height to 16 for xl
  };

  const svgLogo = (
    <svg
      width="1000" // Updated width to 1000
      height="250" // Updated height to 250
      viewBox="0 0 905 251"
      fill="none"
  ...
  return (
    <h1 className="blog-main-logo">
      <Link
        className={`blog-logo focus-ring-base flex flex-row items-center focus-ring-colors-base ${logoSizes[size]}`}
