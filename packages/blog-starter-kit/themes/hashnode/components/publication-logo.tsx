...
const CustomLogo = ({ publication, logoSrc, size = 'lg', isPostPage }: {
  ...
  const { theme } = useTheme();
  const blogTitle = generateBlogTitleWithoutDisplayTitle(publication);
  const darkLogoSrc = publication.preferences.darkMode?.logo || logoSrc; // Use logoSrc as fallback
    ...
    <h1 className="blog-main-logo">
      ...
      >
        <CustomImage
          priority
          className="block w-full"
          src={resizeImage(darkLogoSrc, { w: 903.95, h: 250, c: 'thumb' })} // Use darkLogoSrc
          originalSrc={darkLogoSrc || ''} // Use darkLogoSrc
          width={1000}
          height={250}
          alt={blogTitle}
        />
